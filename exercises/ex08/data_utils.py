...
def filter(dictionary: dict[str, int]) -> dict[str, int]:
    """Given a dictionary with strings as keys and integers as values, this function will produce a dictionary that displays all majors that have 50 or more students currently taking COMP110."""
    result: dict[str, int] = {}
    for major in dictionary:
        if dictionary[major] >= 50:
            result[major] = dictionary[major]
    return result